...
idle_limit=20 #seconds after which we mark as distracted
is_idle=False
...
'''def idle_times():
    global idle_limit,idle_warning_time
    a=int(input("Enter the minutes you want to get notification at : "))
    idle_warning_time=a/60
    idle_limit=idle_warning_time + 300'''
...
def on_activity(_):
    global last_activity,is_idle,warning_given
    last_activity=time.time()
    if is_idle:
        print("\n Back to Focus Mode !")
        show_notification("Focus Restored", "Good job! You're back on track.")
        time.sleep(1)  # brief delay to avoid rapid-fire messages
    # Reset both flags when user is active again
    is_idle = False
    warning_given = False
...
def main():
    global last_activity, is_idle,stop_requested,warning_given
    clear_screen()
    ...
    while not stop_requested:
        ...
        if inactive_for > idle_limit:
            if not is_idle:
                ...
                (r"C:\Users\Acer\Desktop\college-project\focus_system\alert2.wav")
                show_notification("Distraction Alert!","You've been idle for a while-Refocus yourself!",r"C:\Users\Acer\Desktop\college-project\focus_system\alert2.wav")
                is_idle = True
                warning_given=False #reset warning for next cycle
            idle_time += 1

        elif inactive_for>idle_warning_time and not warning_given:
            print("\n Show that you're present!")
            show_notification("Are you still there ? ","Move your mouse or press any key to show that you are focused",r"C:\Users\Acer\Desktop\college-project\focus_system\alert1.wav")
            warning_given=True
